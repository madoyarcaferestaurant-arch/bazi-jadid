...
class Bonfire extends GameDecoration with Sensor<DwarfWarrior> {
  ...
  Bonfire(this.ref, {required Vector2 position})
    : super.withAnimation(
        ...
        size: Vector2(Globals.tileSize, Globals.tileSize + _positionBuffer),
        position: Vector2(position.x, position.y - _positionBuffer),
      ) {
  ...
  void onContact(DwarfWarrior component) {
    component.showDamage(
      _damage,
      config: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold),
    );
    component.playSoundEffect(Globals.audio.flame, ref);

    component.playOnceOther(other: PlatformAnimationsOther.hurt);
